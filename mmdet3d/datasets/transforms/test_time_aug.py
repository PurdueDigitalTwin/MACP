...
class MultiScaleFlipAug3D(BaseTransform):
    ...
    def __init__(
        ...
        self.img_scale = (
            img_scale if isinstance(img_scale, list) else [img_scale])
    ...
    def transform(self, results: Dict) -> List[Dict]:
        ...
        pcd_horizontal_flip_aug = ([False, True] if self.flip
                                   and self.pcd_horizontal_flip else [False])
        pcd_vertical_flip_aug = ([False, True] if self.flip
                                 and self.pcd_vertical_flip else [False])
