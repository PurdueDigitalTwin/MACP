...
class RandomDropPointsColor(BaseTransform):
    ...
    def __init__(self, drop_ratio: float = 0.2) -> None:
        assert (isinstance(drop_ratio, (int, float)) and
                0 <= drop_ratio <= 1), f'invalid drop_ratio value {drop_ratio}'
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        assert (points.attribute_dims is not None and 'color'
                in points.attribute_dims), 'Expect points have color attribute'
    ...
    def __init__(
        ...
        if not isinstance(jitter_std, seq_types):
            assert isinstance(
                jitter_std,
                (int,
                 float)), f'unsupported jitter_std type {type(jitter_std)}'
        ...
        if clip_range is not None:
            if not isinstance(clip_range, seq_types):
                assert isinstance(
                    clip_range,
                    (int,
                     float)), f'unsupported clip_range type {type(clip_range)}'
...
class GlobalAlignment(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        assert (
            'axis_align_matrix'
            in results), 'axis_align_matrix is not provided in GlobalAlignment'

        axis_align_matrix = results['axis_align_matrix']
        assert axis_align_matrix.shape == (
            4,
            4,
...
class GlobalRotScaleTrans(BaseTransform):
    ...
    def __init__(
        ...
        if not isinstance(rot_range, seq_types):
            assert isinstance(
                rot_range,
                (int, float)), f'unsupported rot_range type {type(rot_range)}'
            rot_range = [-rot_range, rot_range]
        self.rot_range = rot_range

        assert isinstance(
            scale_ratio_range, seq_types
    ...
    def _rot_bbox_points(self, input_dict: dict) -> None:
        ...
        if ('gt_bboxes_3d' in input_dict
                and len(input_dict['gt_bboxes_3d'].tensor) != 0):
...
class PointSample(BaseTransform):
    ...
    def _points_random_sampling(
        ...
        if isinstance(num_samples, float):
            ...
            num_samples = int(
                np.random.uniform(self.num_points, 1.0) * points.shape[0])
    ...
    def _patch_points_sampling(
        ...
        for _ in range(self.num_try):
            ...
            if self.ignore_index is None:
                ...
                flag2 = (
                    np.sum(cur_sem_mask != self.ignore_index) /
    ...
    def transform(self, results: dict) -> dict:
        ...
        if isinstance(self.down_ratio, tuple):
            ...
            trans_mat = self._get_transform_matrix(
                ...
                size,
                (
                    self.img_scale[0] // self.down_ratio,
...
class RandomCrop3D(RandomCrop):
    ...
    def __init__(
            ...
            rel_offset_h: tuple = (0.0, 1.0),
            rel_offset_w: tuple = (0.0, 1.0),
    ) -> None:
        super().__init__(
            crop_size=crop_size,
            crop_type=crop_type,
...
class MultiViewWrapper(BaseTransform):
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        for key in self.collected_keys:
            if key not in input_dict or not isinstance(input_dict[key], list):
                input_dict[key] = []
        prev_process_dict = {}
...
class PolarMix(BaseTransform):
    ...
    def __init__(
        ...
        assert is_list_of(instance_classes,
                          int), 'instance_classes should be a list of int'
...
class LaserMix(BaseTransform):
    ...
    def __init__(
        ...
        assert is_list_of(num_areas, int), 'num_areas should be a list of int.'
