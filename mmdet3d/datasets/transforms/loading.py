    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        if self.num_ref_frames > 0:
            ...
            if num_frames == 0:  # no previous frame, then copy cur frames
                ...
                if self.test_mode:
                    choices = (
                        np.arange(num_frames - self.num_ref_frames, num_frames)
                        + 1)
                # NOTE: +1 is for selecting previous frames
                else:
                    choices = (
                        np.random.choice(
                            num_frames, self.num_ref_frames, replace=False) +
                        1)
                ...
                if key in results:
                    ...
                    for choice_idx in range(1, len(choices)):
                        ...
                        for result_idx in range(
                            ...
                            results[key][result_idx] = results[key][
                                result_idx].dot(cur2prev)
...
class PointSegClassMapping(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'eval_ann_info' in results:
            ...
            results['eval_ann_info'][
                'pts_semantic_mask'] = converted_pts_sem_mask
...
class NormalizePointsColor(BaseTransform):
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        assert (points.attribute_dims is not None
                and 'color' in points.attribute_dims.keys()
                ), 'Expect points have color attribute'
        if self.color_mean is not None:
            points.color = points.color - points.color.new_tensor(
                self.color_mean)
...
class MonoDet3DInferencerLoader(BaseTransform):
    ...
    def transform(self, single_input: dict) -> dict:
        ...
        assert ('calib' in single_input and 'img'
                in single_input), "key 'calib' and 'img' must be in input dict"
...
class MultiModalityDet3DInferencerLoader(BaseTransform):
    ...
    def transform(self, single_input: dict) -> dict:
        ...
        Returns:
            ...
            information.
        """
        assert ('points' in single_input and 'img' in single_input and 'calib'
                in single_input), "key 'points', 'img' and 'calib' must be "
