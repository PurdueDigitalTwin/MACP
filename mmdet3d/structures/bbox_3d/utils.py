...
def rotation_3d_in_axis(
    ...
    assert (len(points.shape) == 3 and len(angles.shape) == 1
            and points.shape[0]
            == angles.shape[0]), ('Incorrect shape of points '
...
def points_cam2img(
    ...
    assert ((d1 == 3 and d2 == 3) or (d1 == 3 and d2 == 4)
            or (d1 == 4 and d2 == 4)), ('The shape of the projection matrix '
...
def mono_cam_box2vis(cam_box):
    ...
    assert isinstance(
        cam_box,
        CameraInstance3DBoxes), 'input bbox should be CameraInstance3DBoxes!'
