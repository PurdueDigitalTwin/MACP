            ...
            def recursive_recover(input_data):
                if isinstance(input_data, (tuple, list)):
                    ...
                    return (tuple(new_data)
                            if isinstance(input_data, tuple) else new_data)
...
class ArrayConverter:
    ...
    def convert(
        ...
        assert (target_type is not None
                or target_array is not None), 'must specify a target'
        if target_type is not None:
            ...
            assert isinstance(
                target_array,
                (np.ndarray, torch.Tensor)), 'invalid target array type'
            if isinstance(target_array, array_type):
                return input_array
            elif isinstance(target_array, np.ndarray):
                converted_array = (
                    input_array.cpu().numpy().astype(target_array.dtype))
            else:
    ...
    def recover(
        ...
        assert isinstance(
            input_array,
            (np.ndarray, torch.Tensor)), 'invalid input array type'
