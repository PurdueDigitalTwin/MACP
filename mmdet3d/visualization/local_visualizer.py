...
class Det3DLocalVisualizer(DetLocalVisualizer):
    ...
    def draw_bev_bboxes(
        ...
        bboxes_3d: BaseInstance3DBoxes,
        scale: int = 15,
        edge_colors: Union[str, Tuple[int], List[Union[str,
                                                       Tuple[int]]]] = 'o',
        line_styles: Union[str, List[str]] = '-',
        line_widths: Union[int, float, List[Union[int, float]]] = 1,
        face_colors: Union[str, Tuple[int], List[Union[str,
                                                       Tuple[int]]]] = 'none',
    ...
    def draw_proj_bboxes_3d(
        ...
        if img_size is not None:
            ...
            valid_point_idx = (
                ...
                & (corners_2d[..., 1] >= 0)
                & (corners_2d[..., 1] <= img_size[1]))  # noqa: E501
    ...
    def _draw_instances_3d(
        ...
        if vis_task in ['lidar_det', 'multi-modality_det']:
            ...
            bbox_color = (
                palette if self.bbox_color is None else self.bbox_color)
        ...
        if vis_task in ['mono_det', 'multi-modality_det']:
            ...
            if isinstance(img, list) or (isinstance(img, (np.ndarray, Tensor))
                ...
                img_size = (img[0].shape[:2] if isinstance(img, list) else
                            img.shape[-2:])  # noqa: E501
                ...
                for i, single_img in enumerate(img):
                    ...
                    bbox_color = (
                        palette
                        if self.bbox_color is None else self.bbox_color)
                    bbox_palette = get_palette(bbox_color, max_label + 1)
                    colors = [bbox_palette[label] for label in labels_3d]
    ...
    def add_datasample(
        ...
        assert vis_task in (
            ...
            'lidar_seg',
            'multi-modality_det',
        ), f'got unexpected vis_task {vis_task}.'
