...
def load_lyft_gts(lyft, data_root, eval_split, logger=None):
    ...
    if eval_split == 'test':
        ...
        assert (len(lyft.sample_annotation) >
                0), 'Error: You are trying to evaluate on the test set \
    ...
    for sample_token in mmengine.track_iter_progress(sample_tokens):
        ...
        for sample_annotation_token in sample_annotation_tokens:
            ...
            sample_annotation = lyft.get('sample_annotation',
...
def lyft_eval(lyft, data_root, res_path, eval_set, output_dir, logger=None):
    ...
    metrics = {}
    average_precisions = get_classwise_aps(gts, predictions, class_names,
                                           iou_thresholds)
