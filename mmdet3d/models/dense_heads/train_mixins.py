...
class AnchorTrainMixin:
    ...
    def anchor_target_3d_single(
        ...
        if isinstance(self.bbox_assigner,
            ...
            assert len(self.bbox_assigner) == len(
                anchors
    ...
    def anchor_target_single_assigner(
        self,
        bbox_assigner,
        anchors,
        gt_instance_3d,
        ...
        if len(gt_instance_3d.bboxes_3d) > 0:
            ...
            pos_inds = (
                torch.nonzero(
                    anchors.new_zeros(
                        (anchors.shape[0], ), dtype=torch.bool) > 0,
                    as_tuple=False,
                ).squeeze(-1).unique())
            neg_inds = (
                torch.nonzero(
                    anchors.new_zeros((anchors.shape[0], ),
                                      dtype=torch.bool) == 0,
                    as_tuple=False,
                ).squeeze(-1).unique())
