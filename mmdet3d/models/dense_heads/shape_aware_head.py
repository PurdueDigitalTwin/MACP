...
class BaseShapeHead(BaseModule):
    ...
    def forward(self, x: Tensor) -> Dict:
        ...
        B = bbox_pred.shape[0]
        cls_score = (
            cls_score.view(-1, self.num_base_anchors, self.num_cls, H,
                           W).permute(0, 1, 3, 4,
                                      2).reshape(B, -1, self.num_cls))
        bbox_pred = (
            bbox_pred.view(-1, self.num_base_anchors, self.box_code_size, H,
                           W).permute(0, 1, 3, 4,
        ...
        if self.use_direction_classifier:
            ...
            dir_cls_preds = (
                dir_cls_preds.view(-1, self.num_base_anchors, 2, H,
                                   W).permute(0, 1, 3, 4, 2).reshape(B, -1, 2))
        ret = dict(
            cls_score=cls_score,
            bbox_pred=bbox_pred,
