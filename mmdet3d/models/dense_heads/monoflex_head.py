...
class MonoFlexHead(AnchorFreeMono3DHead):
    ...
    def __init__(
        ...
        loss_cls: dict = dict(type='mmdet.GaussianFocalLoss', loss_weight=1.0),
        loss_bbox: dict = dict(type='mmdet.IoULoss', loss_weight=0.1),
        loss_dir: dict = dict(type='MultiBinLoss', loss_weight=0.1),
        loss_keypoints: dict = dict(type='mmdet.L1Loss', loss_weight=0.1),
        loss_dims: dict = dict(type='mmdet.L1Loss', loss_weight=0.1),
        loss_offsets_2d: dict = dict(type='mmdet.L1Loss', loss_weight=0.1),
        loss_direct_depth: dict = dict(type='mmdet.L1Loss', loss_weight=0.1),
        loss_keypoints_depth: dict = dict(
            type='mmdet.L1Loss', loss_weight=0.1),
        loss_combined_depth: dict = dict(type='mmdet.L1Loss', loss_weight=0.1),
        ...
        for i in range(len(self.group_reg_dims)):
            ...
            for j, conv_reg in enumerate(self.conv_regs[i]):
                ...
                if self.use_edge_fusion and (i, j) in self.edge_fusion_inds:
                    ...
                    out_reg = getattr(self, f'edge_fuse_reg_{i}_{j}')(
                        reg_feat,
                        out_reg,
                        edge_indices,
                        edge_lens,
    ...
    def _decode_heatmap(
        ...
        *batch_dets, topk_ys, topk_xs = get_topk_from_heatmap(
        ...
        pred_locations = self.bbox_coder.decode_location(
            pred_base_centers_2d,
            preds['offsets_2d'],
            preds['combined_depth'],
            cam2imgs,
            downsample_ratio,
        )
        ...
        for batch_id in range(batch_size):
            ...
            for j, base_center2d in enumerate(gt_base_centers_2d):
                if trunc_masks[j]:
                    ...
                    scale_box_w = min(
                        base_center2d_x_int - gt_bboxes[j][0],
                        gt_bboxes[j][2] - base_center2d_x_int,
                    )
                    scale_box_h = min(
                        base_center2d_y_int - gt_bboxes[j][1],
    ...
    def loss_by_feat(
        ...
        loss_keypoints_depth = self.loss_keypoint_depth(
            preds['keypoints_depth'][depth_mask],
            depth_target[depth_mask],
