...
class PGDHead(FCOSMono3DHead):
    ...
    def __init__(
        ...
        loss_bbox2d: dict = dict(
            type='mmdet.SmoothL1Loss', beta=1.0 / 9.0, loss_weight=1.0),
        loss_consistency: dict = dict(type='mmdet.GIoULoss', loss_weight=1.0),
        ...
        **kwargs,
        ...
        if self.division == 'uniform':
            self.num_depth_cls = (
                int((depth_range[1] - depth_range[0]) / depth_unit) + 1)
    ...
    def forward_single(self, x: Tensor, scale: Scale,
        ...
        max_regress_range = (
            stride * self.regress_ranges[0][1] / self.strides[0])
    ...
    def get_proj_bbox2d(
        ...
        for idx in range(num_imgs):
            ...
            cam2img[:view_shape[0], :
                    view_shape[1]] = pos_strided_bbox_preds.new_tensor(
                        views[idx])
            ...
            pos_bbox_targets_3d[mask, 6] = (
                torch.atan2(centers2d_targets[:, 0] - cam2img[0, 2],
        ...
        if with_kpts:
            norm_strides = (
                pos_strides * self.regress_ranges[0][1] / self.strides[0])
    ...
    def loss_by_feat(
        ...
        """
        assert (
            len(cls_scores) == len(bbox_preds) == len(dir_cls_preds) ==
        ...
        if num_pos > 0:
            ...
            if self.pred_keypoints:
                ...
                (
                    ...
                    kpts_targets,
                ) = self.get_proj_bbox2d(
                    *proj_bbox2d_inputs, with_kpts=True)
    ...
    def _predict_by_feat_single(
        ...
        for (
            ...
            depth_cls_score = (
                F.softmax(depth_cls_pred, dim=-1).topk(k=2,
                                                       dim=-1)[0].mean(dim=-1))
            if self.weight_dim != -1:
