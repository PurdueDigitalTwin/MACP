...
class GeneralSamplingModule(nn.Module):
    ...
    def forward(self, xyz: Tensor, features: Tensor,
        ...
        new_xyz = (
            gather_points(xyz_t, sample_inds).transpose(1, 2).contiguous())
...
class GroupFree3DHead(BaseModule):
    ...
    def __init__(
        ...
        if isinstance(transformerlayers, dict):
            ...
            assert (isinstance(transformerlayers, list)
                    and len(transformerlayers) == num_decoder_layers)
        ...
        **kwargs,
        ...
        for prefix in prefixes:
            ...
            if self.size_cls_agnostic:
                ...
                losses[f'{prefix}size_class_loss'] = (
                    size_class_loss / num_stages)
                ...
                one_hot_size_targets_expand = (
                    one_hot_size_targets.unsqueeze(-1).expand(-1, -1, -1,
                                                              3).contiguous())
    ...
    def _get_targets_single(
        ...
        pts_instance_label = (
            points.new_zeros([num_points], dtype=torch.long) - 1)
        ...
        if self.bbox_coder.with_rot:
            ...
            for i in range(gt_labels_3d.shape[0]):
                ...
                votes = (
                    gt_bboxes_3d.gravity_center[i].unsqueeze(0) -
                    selected_points[:, :3])
