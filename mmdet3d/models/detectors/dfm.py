...
class DfM(BaseDetector):
    ...
    def with_depth_head_2d(self):
        ...
        return (hasattr(self, 'depth_head_2d')
                and self.depth_head_2d is not None)
    ...
    def extract_feat(self, img, img_metas):
        ...
        pad_prev_cam2global = (
            torch.eye(4)[None, None].expand(batch_size, N - 1, 4,
                                            4).to(img.device))
        pad_prev_cam2global[:, :, :prev_poses.shape[-2], :prev_poses.
                            shape[-1]] = prev_poses
        pad_cur_cam2global = (
            torch.eye(4)[None, None].expand(batch_size, 1, 4,
                                            4).to(img.device))
