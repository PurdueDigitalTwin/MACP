...
class ImVoteNet(Base3DDetector):
    ...
    clouds, which explicitly extract both geometric and semantic features from
    the 2D images. It leverage camera parameters to lift these features to 3D.
    A multi-tower training scheme also improve the synergy of 2D-3D feature
    fusion.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if img_rpn_head is not None:
            rpn_train_cfg = (
                train_cfg.img_rpn if train_cfg is not None else None)
        ...
        if img_roi_head is not None:
            rcnn_train_cfg = (
                train_cfg.img_rcnn if train_cfg is not None else None)
            img_roi_head.update(
        ...
        for key in list(state_dict):
            for module_name in module_names:
                if (key.startswith(module_name)
                        and ('img_' + key) not in state_dict):
    ...
    def with_img_bbox(self):
        ...
        return (hasattr(self, 'img_roi_head')
                and self.img_roi_head.with_bbox) or (hasattr(
                    self, 'img_bbox_head') and self.img_bbox_head is not None)

    @property
    def with_img_bbox_head(self):
        """bool: Whether the detector has a 2D image box head (not roi)."""
        return (hasattr(self, 'img_bbox_head')
    ...
    def with_pts_bbox(self):
        ...
        return (hasattr(self, 'pts_bbox_head')
                and self.pts_bbox_head is not None)
