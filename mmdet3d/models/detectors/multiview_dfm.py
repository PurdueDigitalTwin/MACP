...
class MultiViewDfM(ImVoxelNet, DfM):
    ...
    def __init__(
        ...
        if backbone_stereo is not None:
            ...
            assert (self.neck.sem_channels[-1] ==
                    self.backbone_stereo.in_sem_channels)
    ...
    def feature_transformation(self, batch_feats, batch_img_metas, num_views,
            ...
            for frame_idx in range(num_frames):
                ...
                for view_idx in range(num_views):
                    ...
                    if 'scale_factor' in img_meta:
                        ...
                        if (isinstance(img_scale_factor, np.ndarray)
        ...
        if self.with_depth_head:
            ...
            for batch_idx in range(volume_feat.shape[0]):
                ...
                for view_idx in range(num_views):
                    img_scale_factor = (
                        img_scale_factors[batch_idx] if self.transform_depth
                        else points.new_tensor([1.0, 1.0]))
                    img_crop_offset = (
                        img_crop_offsets[batch_idx] if self.transform_depth
                        else points.new_tensor([0.0, 0.0]))
                    img_flip = (
                        img_flips[batch_idx]
                        if self.transform_depth else False)
                    img_pad_shape = (
