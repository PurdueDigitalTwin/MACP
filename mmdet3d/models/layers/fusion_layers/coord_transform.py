...
def apply_3d_transformation(pcd: Tensor,
    ...
    pcd_scale_factor = (
        ...
        if 'pcd_scale_factor' in img_meta else 1.0)
    ...
    pcd_horizontal_flip = (
        img_meta['pcd_horizontal_flip']
        if 'pcd_horizontal_flip' in img_meta else False)

    pcd_vertical_flip = (
        img_meta['pcd_vertical_flip']
        if 'pcd_vertical_flip' in img_meta else False)

    flow = (
        img_meta['transformation_3d_flow']
    ...
    horizontal_flip_func = (
        partial(pcd.flip, bev_direction='horizontal')
        if pcd_horizontal_flip else lambda: None)
    vertical_flip_func = (
        partial(pcd.flip, bev_direction='vertical')
...
def bbox_2d_transform(img_meta: dict, bbox_2d: Tensor,
    ...
    (
        ...
        img_flip,
        img_crop_offset,
    ) = extract_2d_info(img_meta, bbox_2d)
...
def coord_2d_transform(img_meta: dict, coord_2d: Tensor,
    ...
    (
        ...
        img_flip,
        img_crop_offset,
    ) = extract_2d_info(img_meta, coord_2d)
