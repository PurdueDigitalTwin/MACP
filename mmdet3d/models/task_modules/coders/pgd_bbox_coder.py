...
class PGDBBoxCoder(FCOS3DBBoxCoder):
    ...
    def decode_2d(
        ...
        if pred_keypoints:
            ...
            bbox[:, self.bbox_code_size:self.bbox_code_size + 16] = torch.tanh(
                scale_kpts(clone_bbox[:,
                                      self.bbox_code_size:self.bbox_code_size +
                                      16]).float())
        ...
        if self.norm_on_bbox:
            ...
            if not training:
                if pred_keypoints:
                    bbox[:, self.bbox_code_size:self.bbox_code_size +
                         16] *= max_regress_range
                if pred_bbox2d:
                    bbox[:, -4:] *= stride
        else:
            if pred_bbox2d:
    ...
    def decode_prob_depth(
        ...
        if division == 'uniform':
            ...
            depth_multiplier = depth_range[0] + (
                depth_range[1] - depth_range[0]) / (num_depth_cls *
                                                    (num_depth_cls - 1)) * (
                                                        split_pts *
                                                        (split_pts + 1))
            prob_depth_preds = (F.softmax(depth_cls_preds.clone(), dim=-1) *
            ...
            log_multiplier = np.log(start) + split_pts * np.log(
                end / start) / (
                    num_depth_cls - 1)
            prob_depth_preds = ((F.softmax(depth_cls_preds.clone(), dim=-1) *
