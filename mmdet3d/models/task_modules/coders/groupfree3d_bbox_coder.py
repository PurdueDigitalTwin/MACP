...
class GroupFree3DBBoxCoder(PartialBinBasedBBoxCoder):
    ...
    def encode(self, gt_bboxes_3d: BaseInstance3DBoxes,
        ...
        size_res_target = (
            gt_bboxes_3d.dims -
            gt_bboxes_3d.tensor.new_tensor(self.mean_sizes)[size_class_target])
    ...
    def split_pred(
        ...
        # (batch_size, num_proposal, 3)
        results[f'{prefix}center_residual'] = reg_preds_trans[
            ..., start:end].contiguous()
        results[f'{prefix}center'] = (
            base_xyz + reg_preds_trans[..., start:end].contiguous())
        start = end

        # decode direction
        end += self.num_dir_bins
        results[f'{prefix}dir_class'] = reg_preds_trans[
            ..., start:end].contiguous()
        start = end
        ...
        if self.size_cls_agnostic:
            ...
            results[f'{prefix}size'] = reg_preds_trans[...,
                                                       start:end].contiguous()
        else:
            end += self.num_sizes
            ...
            results[
                f'{prefix}size_res'] = size_res_norm * mean_sizes.unsqueeze(
                    0).unsqueeze(0)
