...
class MonoFlexCoder(BaseBBoxCoder):
    ...
    def decode(
        ...
        if self.combine_depth:
            ...
            pred_combined_depth = self.combine_depths(pred_depth,
    ...
    def decode_location(
        ...
        # (N, 4, 1)
        centers2d_extend = torch.cat(
            (centers2d_img, centers2d_img.new_ones(N, 1)), dim=1).unsqueeze(-1)
    ...
    def keypoints2depth(
        ...
        group1_index: List[Tuple[int]] = [(2, 6), (1, 5)],
    ) -> Tuple[Tensor]:
        ...
        corner_group0_height = (
            keypoints2d[:, group0_index[0], 1] -
            keypoints2d[:, group0_index[1], 1])
        corner_group1_height = (
            keypoints2d[:, group1_index[0], 1] -
    ...
    def decode_orientation(self, ori_vector: Tensor,
        ...
        if self.multibin:
            ...
            for i in range(self.num_dir_bins):
                ...
                orientations[mask_i] = (
                    pred_bin_offset[:, 0].atan2(pred_bin_offset[:, 1]) +
                    self.bin_centers[i])
        else:
