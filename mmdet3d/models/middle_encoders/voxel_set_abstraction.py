...
def bilinear_interpolate_torch(inputs: Tensor, x: Tensor, y: Tensor) -> Tensor:
    ...
    ans = (
        torch.t(torch.t(Ia) * wa) + torch.t(torch.t(Ib) * wb) +
...
class VoxelSetAbstraction(BaseModule):
    ...
    def get_voxel_centers(self, coors: torch.Tensor,
        ...
        voxel_size = (
            torch.tensor(self.voxel_size, device=voxel_centers.device).float()
            * scale_factor)
    ...
    def forward(
        ...
        key_xyz_batch_cnt = (
            key_xyz.new_zeros(batch_size).int().fill_(num_keypoints))
