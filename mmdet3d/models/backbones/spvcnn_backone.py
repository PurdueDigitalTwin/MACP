...
def voxel_to_point(
    ...
    """
    if (points.idx_query is None or points.weights is None
            or points.idx_query.get(voxels.s) is None
            or points.weights.get(voxels.s) is None):
        ...
        weights = (
            F.calc_ti_weights(points.C, idx_query,
...
def point_to_voxel(voxels: SparseTensor, points: PointTensor) -> SparseTensor:
    ...
    """
    if (points.additional_features is None
            or points.additional_features.get('idx_query') is None
            or points.additional_features['idx_query'].get(voxels.s) is None):
        pc_hash = F.sphash(
            torch.cat(
                [
                    torch.floor(points.C[:, :3] / voxels.s[0]).int() *
