...
class H3DBboxHead(BaseModule):
    ...
    def forward(self, feats_dict: dict):
        ...
        obj_surface_center = (
            obj_surface_center.reshape(batch_size, -1, 6,
                                       3).transpose(1, 2).reshape(
                                           batch_size, -1, 3))
        obj_line_center = (
            obj_line_center.reshape(batch_size, -1, 12,
                                    3).transpose(1,
                                                 2).reshape(batch_size, -1, 3))
        ...
        line_feature = torch.cat(
            (
                line_feature.new_zeros(
                    (batch_size, 12, line_feature.shape[2])),
                line_feature,
            ),
            dim=1,
        )
        ...
        ret_dict[
            'semantic_matching_score'] = semantic_matching_score.transpose(
                2, 1)
    ...
    def loss_by_feat(self, points: List[torch.Tensor], feats_dict: dict,
        ...
        primitive_matching_loss = (
            (objectness_loss_refine * cues_match_mask).sum() /
        ...
        pred_obj_surface_center = (
            pred_obj_surface_center.reshape(batch_size, -1, 6, 3).transpose(
                1, 2).reshape(batch_size, -1, 3))
        pred_obj_line_center = (
            pred_obj_line_center.reshape(batch_size, -1, 12, 3).transpose(
                1, 2).reshape(batch_size, -1, 3))
    ...
    def multiclass_nms_single(
        ...
        nms_selected = aligned_3d_nms(
            minmax_box3d[nonempty_box_mask],
