...
class PointRCNNBboxHead(BaseModule):
    ...
    def forward(self, feats: Tensor) -> Tuple[Tensor]:
        ...
        xyz_input = (
            input_data[..., 0:self.in_channels].transpose(
                1, 2).unsqueeze(dim=3).contiguous().clone().detach())
        xyz_features = self.xyz_up_layer(xyz_input)
        rpn_features = (
            input_data[..., self.in_channels:].transpose(1,
                                                         2).unsqueeze(dim=3))
    ...
    def loss(
        ...
        bbox_weights_flat = (
            bbox_weights[pos_inds].view(-1, 1).repeat(1,
                                                      pos_bbox_pred.shape[-1]))
    ...
    def multi_class_nms(
        ...
        assert (box_probs.shape[1] == self.num_classes
        ...
        score_thresh = (
            score_thr if isinstance(score_thr, list) else
            [score_thr for x in range(self.num_classes)])
        nms_thresh = (
            nms_thr if isinstance(nms_thr, list) else
            [nms_thr for x in range(self.num_classes)])
        ...
        keep = (
            torch.cat(selected_list, dim=0) if len(selected_list) > 0 else [])
        return keep
