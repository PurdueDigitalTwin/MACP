...
class PrimitiveHead(BaseModule):
    ...
    def get_targets(
        ...
        gt_primitive_semantic = (
            seed_gt_sem[:, :,
                        3:3 + self.num_dims].view(batch_size * num_proposal, 1,
                                                  self.num_dims).contiguous())
    ...
    def get_targets_single(
        ...
        for i, i_instance in enumerate(instance_labels):
            ...
            if (self.check_horizon(upper_points)
            ...
            if (self.primitive_mode == 'z'
                    ...
                    and point2plane_dist[selected].var() <
    ...
    def primitive_decode_scores(self, predictions: torch.Tensor,
        ...
        if self.primitive_mode in ['z', 'xy']:
            ret_dict['size_residuals_' +
                     self.primitive_mode] = pred_transposed[:, :, 3:3 +
                                                            self.num_dims]

        ret_dict['sem_cls_scores_' +
                 self.primitive_mode] = pred_transposed[:, :,
                                                        3 + self.num_dims:]
    ...
    def match_point2line(
        ...
        if with_yaw:
            ...
            for pair_index in corners_pair[mode]:
                selected = (
                    self.point2line_dist(points, corners[pair_index[0]],
                                         corners[pair_index[1]]) <
            ...
            sel1 = (
                torch.abs(points[:, 0] - xmin) < self.train_cfg['line_thresh'])
            sel2 = (
                torch.abs(points[:, 0] - xmax) < self.train_cfg['line_thresh'])
            sel3 = (
                torch.abs(points[:, 1] - ymin) < self.train_cfg['line_thresh'])
            sel4 = (
                torch.abs(points[:, 1] - ymax) < self.train_cfg['line_thresh'])
    ...
    def match_point2plane(self, plane: torch.Tensor,
        ...
        selected = (
            torch.abs(point2plane_dist - min_dist) <
            self.train_cfg['dist_thresh'])
    ...
    def _assign_primitive_line_targets(
        ...
        assert (len(corners_pair) == len(point2line_matching) ==
                len(center_axises))
