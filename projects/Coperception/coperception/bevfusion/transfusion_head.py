...
from projects.Coperception.coperception.peft.tools import (freeze_module,
                                                           get_peft_layer)
...
class ConvFuser(nn.Sequential):
    ...
    def forward(self, inputs: List[torch.Tensor]) -> torch.Tensor:
        ...
        if all([x.shape[0] == batch_size for x in inputs]):
            return super().forward(torch.cat(inputs, dim=1))
        else:
            print(f'Warning: batch size mismatch, '
                  f'expected {batch_size}, got {[x.shape[0] for x in inputs]} '
                  f'Skipping the ConvFuser layer.')
            return inputs[0]
...
class TransFusionHead(nn.Module):
    ...
    def predict_by_feat(self,
        ...
        assert (len(rets) == 1
                ), f'only support one layer now, but get {len(rets)} layers'
        ...
        for idx_layer in range(
            ...
            if 'vel' in preds_dict.keys():
                ...
                preds = torch.cat(
                    ...
                    dim=1,
                ).permute(0, 2, 1)  # [BS, num_proposals, code_size]
            ...
            layer_reg_weights = (
                layer_bbox_weights *
                layer_bbox_weights.new_tensor(code_weights)  # noqa: E501
            )
...
@MODELS.register_module()
class TransFusionHeadPEFT(TransFusionHead):

    def __init__(
        self,
        num_proposals: int = 128,
        auxiliary: bool = True,
        in_channels: int = 128 * 3,
        hidden_channel: int = 128,
        num_classes: int = 4,
        # config for Transformer
        num_decoder_layers: int = 3,
        decoder_layer: ConfigType = dict(),
        num_heads: int = 8,
        nms_kernel_size: int = 1,
        ...
        loss_bbox: ConfigType = dict(type='mmdet.L1Loss', reduction='mean'),
        loss_heatmap: ConfigType = dict(
            type='mmdet.GaussianFocalLoss', reduction='mean'),
        # others
        train_cfg: ConfigType = None,
        test_cfg: ConfigType = None,
        bbox_coder: ConfigType = None,
        peft_cfg: OptPEFTConfig = None,
    ):
        super().__init__(num_proposals, auxiliary, in_channels, hidden_channel,
                         num_classes, num_decoder_layers, decoder_layer,
                         num_heads, nms_kernel_size, bn_momentum, common_heads,
                         num_heatmap_convs, conv_cfg, norm_cfg, bias, loss_cls,
                         loss_bbox, loss_heatmap, train_cfg, test_cfg,
                         bbox_coder)
        ...
        if peft_cfg is not None:
            ...
            freeze_module(self.shared_conv)

            # freeze_module(self)
            # unfreeze_module_with_name(self, 'peft_layers')
            # unfreeze_module(self.class_encoding)
            # unfreeze_module(self.prediction_heads)

    def loss(self, batch_feats, batch_data_samples):
        """Loss function for CenterHead, same as parent class.

        Args:
            batch_feats: List[Tensor]: Features in a batch. Tensor: (B, C, H, W)
            batch_data_samples List[Det3DDataSample]: The Data Samples. len(batch_data_samples) == B
        Returns:
            dict[str:torch.Tensor]: Loss of heatmap and bbox of each task.
        """
        batch_input_metas, batch_gt_instances_3d = [], []
    ...
    def forward_single(self, inputs, metas):
        """Forward function for CenterHead.

        Args:
            inputs (Tensor): Input feature map with shape [B, 512(C), H, W].
        Returns:
            list[dict]
        """
        batch_size = inputs.shape[0]
        fusion_feat = self.shared_conv(inputs)  # [B, 128(C), H, W].

        # -- PEFT -- #
        if hasattr(self, 'peft_cfg'):
            prefix = '_'.join([self.__class__.__name__, 'shared_conv'])
            peft_layer = get_peft_layer(self.peft_cfg, self.peft_layers,
                                        prefix)
            fusion_feat = peft_layer(fusion_feat)
        # ---------- #

        # Image feature to BEV feature
        fusion_feat_flatten = fusion_feat.view(batch_size,
                                               fusion_feat.shape[1],
                                               -1)  # [B, C, H*W]
        bev_pos = self.bev_pos.repeat(batch_size, 1, 1).to(fusion_feat.device)
        ...
        with torch.autocast('cuda', enabled=False):
            if not hasattr(self, 'peft_cfg'):
                ...
                # -- PEFT -- #
                prefix = '_'.join([self.__class__.__name__, 'heatmap_head'])
                x = fusion_feat.float()
                for i, f in enumerate(self.heatmap_head):
                    x = f(x)
        ...
        # Apply NMS in BEV+
        heatmap = dense_heatmap.detach().sigmoid()  # [B, NC, H, W]
        padding = self.nms_kernel_size // 2
        local_max = torch.zeros_like(heatmap)
        local_max_inner = F.max_pool2d(
            heatmap, kernel_size=self.nms_kernel_size, stride=1, padding=0)
        local_max[:, :, padding:(-padding),
                  padding:(-padding)] = local_max_inner
        heatmap = heatmap * (heatmap == local_max).float()
        heatmap = heatmap.view(batch_size, heatmap.shape[1],
                               -1)  # [B, NC, H*W]
        # Top num_proposals among all classes
        top_proposals = heatmap.view(batch_size, -1).argsort(
            dim=-1, descending=True)[..., :self.num_proposals]
        ...
        self.query_labels = top_proposals_class
        # Add category info to query feature
        one_hot = F.one_hot(
            top_proposals_class,
            num_classes=self.num_classes).permute(0, 2, 1)
        query_cat_encoding = self.class_encoding(one_hot.float())
        ...
        # --------------------- Transformer Decoder (Fusion feature as K, V) --------------------- #
        ret_dicts = []
        for i in range(self.num_decoder_layers):
            query_feat = self.decoder[i](
                query_feat,
                key=fusion_feat_flatten,
                query_pos=query_pos,
                key_pos=bev_pos)  # [B, C, num_proposals]
            ...
            if self.test_cfg['dataset'] in ['OpenV2V', 'V2V4Real']:
                del res_layer['vel']
